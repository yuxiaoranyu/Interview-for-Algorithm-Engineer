...
限制是跨框架能力不如ONNX，面向非PyTorch推理引擎的通用性较弱。
...
常见导出方式是使用 `torch.onnx.export`。基本示例：
...
EP回退可以提升兼容性，但也可能带来性能问题。例如一个模型大部分算子在GPU上执行，少数算子回退到CPU，就可能产生CPU/GPU之间的数据拷贝，导致端到端延迟明显变差。
...
- trace导出可能只记录某一次输入经过的分支
- script虽然能保留部分控制流，但目标推理后端不一定支持
