...
> 本节围绕传统深度学习模型的推理部署展开，以ONNX和TensorRT这两大主要技术为主线，覆盖ONNX导出流程、中间表示原理、ONNX Runtime推理引擎、TensorRT高性能推理以及Triton Inference Server模型服务化部署等内容。读完本节后，读者可以系统理解PyTorch模型如何通过ONNX进行跨平台部署，掌握TensorRT的图优化、kernel选择、低精度推理和Engine构建流程，并能够从模型导出、推理加速、服务化部署等角度分析和解决传统模型部署中的常见问题。

[1.什么是ONNX？它有什么作用？](#1.什么是ONNX？它有什么作用？)
  - [面试问题：ONNX为什么能作为不同训练框架和推理框架之间的桥梁？](#面试问题-onnx为什么能作为不同训练框架和推理框架之间的桥梁)
  - [面试问题：ONNX由哪些组件组成？Graph、Node、Tensor和Opset分别表示什么？](#面试问题-onnx由哪些组件组成graph-node-tensor和opset分别表示什么)
...
[6.TensorRT推理引擎相关高频问题](#6.TensorRT推理引擎相关高频问题)
  - [面试问题：TensorRT的加速原理是什么？为什么在NVIDIA GPU上性能好？](#面试问题-tensorrt的加速原理是什么为什么在nvidia-gpu上性能好)
...
ONNX能作为桥梁，主要原因是它统一描述了模型推理所需的关键信息：
...
<h2 id="面试问题-onnx由哪些主要组件组成graph-node-tensor和opset分别表示什么">面试问题：ONNX由哪些主要组件组成？Graph、Node、Tensor和Opset分别表示什么？</h2>
...
中间表示是为了把模型从训练代码中抽离出来，只保留推理所需的信息：
...
可以总结为：**模型中间表示的主要优势是解耦训练和推理，把灵活研发阶段的模型转换成更适合优化、迁移和上线的标准计算图。它让推理框架更容易做图优化、硬件适配和服务化部署。**
...
> PyTorch模型导出ONNX的主要流程是：将模型切换到推理模式，准备符合真实输入形状的dummy input，调用ONNX导出接口生成ONNX文件，再通过ONNX checker、ONNX Runtime推理和输出对齐验证导出结果是否正确。
...
如果不设置dynamic_axes，导出的ONNX通常会把dummy input中的shape固化。例如dummy input是 `[1, 3, 224, 224]`，那么导出模型只接受固定batch和固定分辨率。
...
> ONNX Runtime是一个跨平台推理运行时，它可以加载ONNX模型，并通过不同Execution Provider把计算分发到CPU、CUDA、TensorRT、OpenVINO等后端执行。它的优势是通用性强、生态成熟、后端可切换，适合传统模型的快速部署和跨硬件推理。
...
|---|---|---|---|
| 优势 | 跨平台、易集成、多EP | NVIDIA GPU性能强 | Intel硬件优化好 |
| 硬件绑定 | 弱 | 强 | 中到强 |
| 部署复杂度 | 低到中 | 中到高 | 中 |
...
很多ONNX输出不一致不是导出的问题，而是输入预处理或模型状态没对齐。
...
<h2 id="面试问题-tensorrt的加速原理是什么？为什么在nvidia-gpu上性能好">面试问题：TensorRT的加速原理是什么？为什么在NVIDIA GPU上性能好？</h2>
...
它的目标是在保证精度可接受的前提下，让模型在NVIDIA GPU上跑得更快、占用更少显存、吞吐更高。
...
可以总结为：**TensorRT性能好的主要原因是它在NVIDIA GPU上做了深度图优化、算子融合、kernel选择、低精度Tensor Core加速和内存规划。它适合追求低延迟和高吞吐的NVIDIA GPU生产部署，但需要处理动态shape、算子支持、Engine兼容和精度校准等工程问题。**
...
> Triton Inference Server是NVIDIA开源的模型推理服务框架，重点解决模型上线后的服务化、并发调度、多模型管理、动态Batch、异构Backend、模型热更新和GPU资源利用问题，是面向生产推理服务的统一服务框架。
...
Triton把不同类型的模型统一托管起来，提供稳定、高并发、可观测、易管理的推理服务。
...
需要注意，实例数量要根据实际算力资源和负载去合理配置。

### 五、Dynamic Batching：动态合并请求

Dynamic Batching是Triton重要的性能优化策略。
...
可以总结为：**Triton的模型仓库用于按目录管理模型和版本；Backend决定模型由TensorRT、ONNX Runtime、PyTorch、Python等哪种运行时执行；Instance Group决定模型实例数量和设备分布；Dynamic Batching负责把短时间内到达的小请求合并成batch执行，以吞吐换取少量排队延迟。这几个概念共同构成了Triton生产推理服务的配置。**
...
这个过程是区分瓶颈在模型执行、请求排队、CPU前后处理、数据拷贝还是GPU资源不足。
