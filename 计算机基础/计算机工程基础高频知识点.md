...
- [27.AIGC服务从本地demo到生产部署的工程链路是什么？](#27.AIGC服务从本地demo到生产部署的工程链路是什么？)
- [28.Docker镜像、模型权重、CUDA、cuDNN、TensorRT版本如何匹配？](#28.Docker镜像模型权重CUDAcuDNNTensorRT版本如何匹配？)
- [29.模型服务可观测性如何设计？](#29.模型服务可观测性如何设计？)
- [30.Agent沙箱、权限、审计、灰度和回滚如何做？](#30.Agent沙箱权限审计灰度和回滚如何做？)
- [31.Linux工具链在AI工程中的高频排查命令有哪些？](#31.Linux工具链在AI工程中的高频排查命令有哪些？)
...
<h1 id='27.AIGC服务从本地demo到生产部署的工程链路是什么？'>27.AIGC服务从本地demo到生产部署的工程链路是什么？</h1>

本地demo只证明“功能能跑”，生产部署要证明“稳定、可扩展、可观测、可恢复、可治理”。一个AIGC服务从demo到生产通常经历以下链路：

1. 模型固化：确定模型版本、权重、tokenizer、配置、依赖版本。
2. 接口封装：提供HTTP/gRPC接口，定义输入输出schema、错误码、超时和鉴权。
3. 容器化：构建镜像，锁定CUDA/cuDNN/TensorRT/Python/C++依赖。
4. 资源评估：压测吞吐、首token延迟、显存占用、CPU占用、最大并发。
5. 调度部署：按GPU型号、显存、模型大小配置实例和副本。
6. 可观测性：接入日志、指标、trace、告警和错误采样。
7. 安全治理：鉴权、限流、内容安全、工具权限、数据脱敏。
8. 灰度发布：小流量验证新模型或新版本，异常可快速回滚。
9. 评测闭环：持续记录线上失败样本，进入离线评测和模型/Prompt优化。

面试中可以用一句话总结：AIGC工程化的核心，是把一次性模型调用变成可长期运行的服务系统。


<h1 id='28.Docker镜像模型权重CUDAcuDNNTensorRT版本如何匹配？'>28.Docker镜像、模型权重、CUDA、cuDNN、TensorRT版本如何匹配？</h1>

AI服务部署最常见的问题之一是版本不匹配。CUDA、GPU驱动、cuDNN、TensorRT、PyTorch/ONNX Runtime、模型权重格式之间存在兼容关系。

关键原则：

1. GPU驱动在宿主机，CUDA运行时通常在容器内。宿主机驱动版本必须支持容器内CUDA版本。
2. TensorRT engine通常和GPU架构、TensorRT版本、部分构建参数强相关，不能随意跨环境复用。
3. PyTorch、ONNX Runtime、TensorRT、cuDNN要选择互相兼容的版本组合。
4. 模型权重、tokenizer、config必须成套管理，不能只替换单个文件。
5. 镜像要记录版本信息，启动时打印关键依赖版本，便于排障。
...
| CUDA | 具体CUDA版本 |
| cuDNN | 具体cuDNN版本 |
| TensorRT | 具体TensorRT版本 |
| 推理框架 | PyTorch / ONNX Runtime / TensorRT-LLM / vLLM |
| 模型权重 | 模型名、量化方式、checkpoint版本 |


<h1 id='29.模型服务可观测性如何设计？'>29.模型服务可观测性如何设计？</h1>

模型服务可观测性至少包括日志、指标、trace和样本四类信息。

| 类型 | 记录内容 | 价值 |
| --- | --- | --- |
| 日志 | request_id、用户、模型、错误码、关键阶段耗时 | 定位单次请求问题 |
| 指标 | QPS、延迟、错误率、token吞吐、GPU利用率、显存 | 判断整体服务健康 |
| Trace | 网关、检索、rerank、模型、工具调用链路 | 定位慢在哪个环节 |
| 样本 | 输入摘要、输出摘要、召回文档、评分 | 进入评测和质量优化 |

LLM服务特别需要记录：

1. 首token延迟。
2. 每秒输出token数。
3. prefill耗时和decode耗时。
4. 输入token数和输出token数。
5. 队列等待时间。
6. 工具调用耗时和失败率。
7. RAG召回TopK和引用命中情况。

注意隐私与合规：日志中不要直接保存敏感prompt、密钥、身份证、手机号等信息。需要保存样本时，应做脱敏、采样和权限控制。


<h1 id='30.Agent沙箱权限审计灰度和回滚如何做？'>30.Agent沙箱、权限、审计、灰度和回滚如何做？</h1>

Agent系统比普通聊天系统更危险，因为它可以调用工具、读写文件、访问网络、执行代码或操作数据库。工程上需要把Agent当成“半自动执行系统”治理。

核心机制：

1. 沙箱：代码执行、shell命令、文件处理在隔离环境中运行，限制CPU、内存、网络、文件系统和执行时间。
2. 权限：按用户、租户、工具、资源设置最小权限。
3. 审计：记录工具调用、参数摘要、结果摘要、操作者、时间、request_id。
4. 策略：危险操作前做规则校验，例如删除文件、发送邮件、改数据库、访问内网。
5. 灰度：新工具、新模型、新Prompt模板先小流量发布。
6. 回滚：保留上一版本模型、配置和工具schema，异常时快速切回。
...
| tool_name | 工具名 |
| allowed_roles | 可调用角色 |
| network_policy | 可访问域名或网段 |
| filesystem_policy | 可读写路径 |
| timeout_ms | 最大执行时间 |
| require_confirmation | 是否需要人工确认 |
| audit_level | 审计等级 |

面试金句：Agent工程不是让模型“更敢做事”，而是让模型在可控边界内做事。


<h1 id='31.Linux工具链在AI工程中的高频排查命令有哪些？'>31.Linux工具链在AI工程中的高频排查命令有哪些？</h1>

AI工程师需要具备基础系统排障能力。常用命令可以按问题分类：
...
1. 服务起不来：先看日志、端口、环境变量、动态库、模型路径。
2. 推理慢：看GPU利用率、CPU占用、队列等待、数据加载、网络延迟。
3. 显存爆：看模型副本数、batch、KV Cache、是否有未释放请求。
4. 请求失败：看网关日志、服务日志、下游工具日志和request_id链路。
5. 磁盘满：看日志滚动、临时文件、生成结果、Docker镜像和已删除但未释放文件。
